...
import { Mail, MailOpen, Lock, Calendar, X, Check, Eye, Sparkles, PenLine } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] pt-28 pb-12 px-4 md:px-6 transition-colors duration-500">
      <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-1000">
        ...
        {/* Filter Section - Minimalist Underline Style */}
            ...
            filteredAndSortedLetters.map((letter) => {
              ...
              return (
                <Card 
                  ...
                  className={cn(
                    "relative aspect-square rounded-[2rem] overflow-hidden transition-all duration-500 group border-none",
                    isAvailable 
                      ? "cursor-pointer hover:shadow-2xl hover:translate-y-[-4px]" 
                      : "cursor-default"
                  )}
                >
                  {/* Card BG with Textures */}
                  <div className={cn(
        ...
        <div className="fixed inset-0 bg-[#2C2C2E]/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={() => setIsWriteModalOpen(false)}>
          <div className="bg-white w-full max-w-xl flex flex-col overflow-hidden shadow-2xl rounded-[2.5rem] animate-in zoom-in-95 duration-300 relative border border-white/20" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-10 pt-4">
              <Textarea placeholder="미래의 당신에게 전하고 싶은 이야기를 적어보세요..." className="w-full border-none focus-visible:ring-0 text-lg min-h-[350px] p-0 bg-transparent placeholder:text-[#8E8E93]/40 placeholder:text-lg leading-relaxed resize-none scrollbar-hide font-medium text-[#2C2C2E]" value={content} onChange={(e) => setContent(e.target.value)} />
            </div>
      ...
      {selectedLetter && (
        <div className="fixed inset-0 bg-[#2C2C2E]/60 backdrop-blur-sm z-[120] flex items-center justify-center p-4 animate-in fade-in duration-300" onClick={() => setSelectedLetter(null)}>
          <div className="bg-white w-full max-w-2xl flex flex-col overflow-hidden shadow-2xl rounded-[2.5rem] animate-in zoom-in-95 duration-300 relative border border-white/20 max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <header className="p-10 border-b border-[#E5E5E1] flex flex-col relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-green-400 to-[#3A7BD5]" />
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-black text-[#8E8E93] uppercase tracking-widest">미래에서 도착한 편지</span>
                </div>
                <button onClick={() => setSelectedLetter(null)} className="p-2.5 hover:bg-[#F5F5F7] rounded-full transition-all border border-[#E5E5E1] shadow-sm">
                  <X className="h-5 w-5 text-[#8E8E93]" />
                </button>
              </div>
              <h2 className="text-3xl font-black text-[#2C2C2E] tracking-tighter leading-tight">
                {selectedLetter.title}
              </h2>
              <div className="flex items-center gap-6 mt-6 text-[11px] font-black text-[#8E8E93] uppercase tracking-widest">
                <div className="flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-[#3A7BD5]" />
            ...
            <div className="flex-1 overflow-y-auto p-12 bg-[#F5F5F7]/30 scrollbar-hide">
              <p className="text-[#2C2C2E] text-xl leading-[2] font-medium whitespace-pre-wrap tracking-tight">
                {selectedLetter.content}
              </p>
            </div>
            <footer className="p-10 border-t border-[#E5E5E1] flex justify-center">
              <Button onClick={() => setSelectedLetter(null)} className="bg-[#2C2C2E] hover:bg-black text-white rounded-2xl px-16 h-14 font-black shadow-xl shadow-black/10 transition-all text-base border-none">
                확인했습니다
              </Button>
            </footer>
          </div>
